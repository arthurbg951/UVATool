...
from .enums import Analise, Support
...
class Process:
    ...
    def __init__(
        ...
        analisys: Analise = Analise.viaRigidezAnalitica,
        verbose=False,
    ) -> None:
        self.__nodes = nodes
    ...
    def __getEquilibriumMatrix(self) -> numpy.array:
        ...
        for element in self.__elements:
            ...
            equilibrium_matrix[0 + node_index, 1 + element_index] = (
                numpy.sin(angle) / length
            )
            equilibrium_matrix[0 + node_index, 2 + element_index] = (
                -numpy.sin(angle) / length
            )
            equilibrium_matrix[1 + node_index, 0 + element_index] = -numpy.sin(angle)
            equilibrium_matrix[1 + node_index, 1 + element_index] = (
                -numpy.cos(angle) / length
            )
            equilibrium_matrix[1 + node_index, 2 + element_index] = (
                numpy.cos(angle) / length
            )
    ...
    def __getFrameStiffnessMatrix(self) -> numpy.array:
        ...
        stifiness_matrix = numpy.zeros(
            (3 * len(self.__elements), 3 * len(self.__elements))
        )
        for i in range(len(self.__elements)):
            ...
            stifiness_matrix[1 + (3 * i), 1 + (3 * i)] = (3 * p1 / (4 - p1 * p2)) * (
                4 * young_module * moment_inertia / length
            )
            stifiness_matrix[1 + (3 * i), 2 + (3 * i)] = (
                3 * p1 * p2 / (4 - p1 * p2)
            ) * (-2 * young_module * moment_inertia / length)
            stifiness_matrix[2 + (3 * i), 1 + (3 * i)] = (
                3 * p1 * p2 / (4 - p1 * p2)
            ) * (-2 * young_module * moment_inertia / length)
            stifiness_matrix[2 + (3 * i), 2 + (3 * i)] = (3 * p2 / (4 - p1 * p2)) * (
                4 * young_module * moment_inertia / length
            )
    ...
    def __getGlobalFrameStiffnessMatrix(self) -> numpy.array:
        ...
        equilibrium_matrix_restriction = numpy.delete(
            equilibrium_matrix_restriction, self.__cuts, 0
        )  # Cut
        ...
        equilibrium_matrix_transpose = (
            equilibrium_matrix_transpose.transpose()
        )  # Transpose
        equilibrium_matrix_transpose = numpy.delete(
            equilibrium_matrix_transpose, self.__cuts, 1
        )  # Cut
    ...
    def __getDisplacement(self) -> numpy.array:
        ...
        displacement = (
            numpy.linalg.inv(self.__global_frame_stiffness) @ self.__nodal_force
        )
        return displacement
    ...
    def __verifySemiFixedSupport(self):
        ...
        if haveSemiFixedSupport:
            ...
            for elemIndex in range(len(self.__elements)):
                node1 = Node(
                    self.__elements[elemIndex].node1.x,
                    self.__elements[elemIndex].node1.y,
                )
                node2 = Node(
                    self.__elements[elemIndex].node2.x,
                    self.__elements[elemIndex].node2.y,
                )
                ...
                if node1.getSupport() == Support.semi_fixed:
                    ...
                    nodePos = (
                        self.__nodes.index(node1, 0, len(self.__nodes))
                        + nodePosCorrection
                    )
                    self.__nodeSemiFixedCuts.append(
                        self.__nodes.index(node1, 0, len(self.__nodes))
                    )
                ...
                if node2.getSupport() == Support.semi_fixed:
                    ...
                    nodePos = (
                        self.__nodes.index(node2, 0, len(self.__nodes))
                        + 1
                        + nodePosCorrection
                    )
                    print(nodePos)
                    self.__nodeSemiFixedCuts.append(
                        self.__nodes.index(node2, 0, len(self.__nodes)) + 1
                    )
        ...
        if self.__nodeSemiFixedCuts != [] and self.__elementSemiFixedCuts != []:
            for i in reversed(range(0, len(self.__internal_forces))):
                if self.__elementSemiFixedCuts.__contains__(i):
                    self.__internal_forces = numpy.delete(
                        self.__internal_forces, [[0 + i], [1 + i], [2 + i]]
                    )
